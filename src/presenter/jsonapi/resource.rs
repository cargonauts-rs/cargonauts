use api::raw::{RawResource, FetchRelationships, ResourceObject};
...
pub struct JsonApiResourceObject<'a, T: RawResource, L: MakeLinks + 'a> {
...
impl<'a, T: RawResource + Represent, L: MakeLinks> Represent for JsonApiResourceObject<'a, T, L> {
...
pub struct JsonApiCollectionObject<'a, T: RawResource, L: MakeLinks + 'a> {
    pub resources: &'a [ResourceObject<T>],
    pub linker: &'a L,
}

impl<'a, T: RawResource + Represent, L: MakeLinks> Represent for JsonApiCollectionObject<'a, T, L> {
    fn repr<S: Serializer>(&self, serializer: &mut S, field_set: Option<&[String]>) -> Result<(), S::Error> {
