...
pub trait Get: Resource {
    fn get(id: Self::Identifier, env: &mut Environment) -> Box<Future<Item = Self, Error = Error>> where Self: Sized;
}
...
pub trait GetOne<R: Relationship>: Resource {
    fn get_one(id: Self::Identifier, env: &mut Environment) -> Box<Future<Item = R::Related, Error = Error>> where Self: Sized;
}
...
pub trait GetMany<R: Relationship>: Resource {
    fn get_many(id: Self::Identifier, env: &mut Environment) -> Box<Stream<Item = R::Related, Error = Error>> where Self: Sized;
}
